...
export const SliderWithInput = (props: SliderWithInputProps) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', margin: '10px 0' }}>
            <h4 style={{ margin: '0', fontSize: '15px', lineHeight: '18px', alignSelf: 'flex-start' }}>
                {props.label}
            </h4>
            <div style={{width:'100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px' }}>
                ...
                <input
                    type="range"
                    value={value}
                    onChange={(e) => handleSliderChange(e as unknown as Event, Number((e.target as HTMLInputElement).value))}
                    min={props.min}
                    max={props.max}
                    step={props.step}
                    style={{
